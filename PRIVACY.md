...
| --- | --- | --- | --- |
| `public/current.json.asc`, `public/b/<bucket>.json.asc` | No. Encrypted to **user public key** (if that env is set). | Latest reading; history batches (mg/dL, trend, timestamp) | Only someone with the **user private key**. Droplet root cannot. |
...
If `PGP_USER_PUBLIC_KEY_PATH` is **unset**, glucose files are encrypted to the **server** key instead. Then the same host that has `private.asc` can decrypt all history. That is the fallback in `App::recipientCrypto()`, not user-custody mode.
