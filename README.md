...
- **User key** (`PGP_USER_PUBLIC_KEY_PATH`) — the public half of a keypair whose private half lives only in your browser. Outbound snapshots are encrypted to this key. A missing file is not a fallback: the poller refuses to start.
...
The dashboard POSTs **only the public key** to `/api/keys`, which writes `data/keys/user-public.asc`. The poller encrypts new snapshots to that key. The private key stays in this browser (IndexedDB) plus your downloaded file. On a public host this POST requires HTTPS.
...
Since the host then holds the matching `private.asc`, it can read published glucose — that is the convenience trade-off of this mode, and it is only ever the result of setting that variable on purpose. Leaving `PGP_USER_PUBLIC_KEY_PATH` empty is not a fallback: the poller refuses to start.
...
| `PGP_USER_PUBLIC_KEY_PATH` | User public key written by `POST /api/keys`. Default `data/keys/user-public.asc`. Required: outbound snapshots are encrypted to it. |
...
`nginx/default.conf` proxies `/api/` to the PHP API on `127.0.0.1:8765`, not to the poller. `/api/keys` stores the user's public key under `data/keys/`. LibreLinkUp login POSTs are forwarded to `AUTH_LISTEN` on loopback. The dashboard posts email/password once; the poller logs into Abbott over HTTPS, writes `data/libre-session.json.asc`, and drops the password. When the session file is missing or the token expires, `status.json.asc` sets `loginRequired` and the connect form comes back.
