...
- **Object naming:** `b/<bucket>.json.enc`, where `bucket = floor(epochSeconds / bucketSeconds) * bucketSeconds` (default `bucketSeconds = 300`). Fixed width, sortable, and computable by the browser from the clock alone.
